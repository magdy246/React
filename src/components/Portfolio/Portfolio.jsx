...
export default function Portfolio() {
  ...
  const photos = [port1, port2, port3, port1, port2, port3];

  const handlePhotoClick = (event) => {
    const clickPhoto = event.currentTarget.getAttribute("data-src");
    setSrc(clickPhoto);
  };
          ...
          <div className="flex flex-row justify-center items-center w-full">
            <div className="mb-8 px-6 flex flex-row flex-wrap">
              {photos.map((photo, index) => (
                <div key={index} className="lg:w-1/3 md:w-1/2 w-full px-3 mt-6">
                  <div className="relative rounded-xl overflow-hidden">
                    ...
                    >
                      <FontAwesomeIcon className="text-white text-7xl" icon={faPlus} />
                    </div>
                    <img className="photo w-full cursor-pointer" src={photo} alt="port" />
                  </div>
                </div>
              ))}
            </div>
